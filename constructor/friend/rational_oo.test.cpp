...
TEST_CASE("Rational number is const-correct") {
    // Check if operator double can be invoked on a const rational_number object
    CHECK(std::is_invocable_v<decltype(&rational_number::operator double), const rational_number>);

    // Define a pointer to the const member function operator[] that takes a char argument
    auto(rational_number::*pf)(char) const = &rational_number::operator[];

    // Check if operator[] can be invoked on a const rational_number object with a char argument
